...
const getOrderStatusByStoreeApi = async ({
  orderBy,
  sortBy,
  q,
  limit,
  ...
  const res = await axiosClient.get(
    `/order/get-orders-status/by-store/${storeId}?q=${q}&sortBy=${sortBy}&orderBy=${orderBy}&page=${page}&limit=${limit}&status=${status}`
  );
  if (res) {
    return res;
  }
...
const updateStatusOrderByStoreApi = async ({ storeId, orderId, status }) => {
  const res = await axiosClient.put(
    `/order/update-status-order/${storeId}/${orderId}?status=${status}`
  );
  if (res) {
    return res;
  }
