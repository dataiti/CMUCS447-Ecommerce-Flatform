...
const getListUserForAdminApi = async ({ orderBy, sortBy, q, limit, page }) => {
  const res = await axiosClient.get(
    `/user/list-users/admin?q=${q}&sortBy=${sortBy}&orderBy=${orderBy}&page=${page}&limit=${limit}`
  );
  if (res) {
    return res;
  }
...
const followStoreApi = async ({ userId, storeId }) => {
  const res = await axiosClient.put(`/user/follow-store/${userId}/${storeId}`);
  if (res) {
    return res;
  }
};

const unFollowStoreApi = async ({ userId, storeId }) => {
  const res = await axiosClient.put(
    `/user/unfollow-store/${userId}/${storeId}`
  );
  if (res) {
    return res;
  }
